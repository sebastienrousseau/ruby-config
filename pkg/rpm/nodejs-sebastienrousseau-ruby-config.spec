...
%global npm_bare      ruby-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-ruby-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Ruby configuration providing standardized RuboCop and StandardRB rules adhering to modern 2026 standards
...
%description
@sebastienrousseau/ruby-config is a shareable configuration preset with no runtime
...
install -pm 0644 .rubocop.yml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 standard.yml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
