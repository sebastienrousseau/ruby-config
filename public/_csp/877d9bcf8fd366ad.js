...
lp=lm?'/ruby-config/'+lm[1]:'/ruby-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/ruby-config/'+lm[1]+'/search-index.json':'/ruby-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
