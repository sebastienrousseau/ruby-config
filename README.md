...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/ruby-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Hardened RuboCop & StandardRB Rules](#hardened-rubocop--standardrb-rules) — before and after rule comparison
...
Install `@sebastienrousseau/ruby-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/ruby-config

# pnpm
pnpm add -D @sebastienrousseau/ruby-config

# yarn
yarn add -D @sebastienrousseau/ruby-config

# bun
bun add -d @sebastienrousseau/ruby-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/ruby-config.git
cd ruby-config
make            # check + test
```
...
- **Target environment**: Optimized for Ruby 3.3+ / RuboCop.
...
```bash
cp node_modules/@sebastienrousseau/ruby-config/.rubocop.yml ./.rubocop.yml
cp node_modules/@sebastienrousseau/ruby-config/standard.yml ./standard.yml
```

### Or point the tool at the installed file

```bash
rubocop --config node_modules/@sebastienrousseau/ruby-config/.rubocop.yml
```
...
`@sebastienrousseau/ruby-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/ruby-config` solves this with three deliberate engineering choices:
...
## Hardened RuboCop & StandardRB Rules

Enforces modern Ruby 3.3+ idioms, frozen string literals, performance optimizations, and deterministic indentation.
...
### After (@sebastienrousseau/ruby-config Enforced)
...
When maintaining unmaintained legacy Ruby 2.x gems with incompatible keyword argument semantics.
